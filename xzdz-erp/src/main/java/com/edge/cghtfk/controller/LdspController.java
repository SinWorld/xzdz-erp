 ...
 */
@Controller
@RequestMapping(value = "cghtfk_ldsp")
public class LdspController {
	...
	// 跳转至领导审批页面
	@RequestMapping(value = "/initLdsp.do")
	public String initLdsp(@RequestParam String objId, String taskId, Model model) {
		String cghtfkdm = objId.substring(objId.indexOf(".") + 1);
		model.addAttribute("cghtfkdm", cghtfkdm);
		model.addAttribute("taskId", taskId);
		return "cghtfk/saveLdsp";
	}

	// 提交表单推动流程
	@RequestMapping(value = "/saveLdsp.do")
	public String saveLdsp(@RequestParam String task_Id, String out_come, String advice_, HttpServletRequest request,
			Model model) {
