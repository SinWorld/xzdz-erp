...
layui.use(['form', 'layedit', 'laydate','transfer','util'], function(){
  ...
  var editIndex = layedit.build('LAY_demo_editor');
	...
	function allStock(transfer){
			$.ajax({
				type : "post",
				url : "<c:url value='/proStock/queryAllStock.do'/>",
				async : false,
				dataType : 'json',
				error : function() {
					alert("出错");
	...
	function CreateTable(strs){
		$.ajax({
			type : "post",
			url : "<c:url value='/proStock/queryChoseStock.do'/>",
			async : false,
			dataType : 'json',
			data:{"strs":strs},
			error : function() {
			...
			success : function(msg) {
				...
				for(var i=0;i<msg.length;i++){
					...
					var addtr = $("<tr>"+
							...
							"<td><input type='text' class='form-control bj' aria-label='' aria-describedby='' value='"+msg[i].stock+"' disabled=''> <input type='hidden' name='stock_Id' value='"+msg[i].stock_Id+"'></td>"+
	...
	function saveSubmit(){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/proStock/saveProStock.do'/>",
			async : false,
