...
<body style="width:100%;padding:0px; margin:0px;text-align: center;" onload="refreshAndClose()">
	<div style="width:1280px;height:auto;padding:0px; margin:0 auto;" id="main">
		<form class="layui-form" action='<c:url value="/matStock/syclrk.do"/>' method="post">
		<input type="hidden" id="url" value='<c:url value="/"/>'>
		<input type="hidden" id="flag" value="${flag}">
		<input type="hidden" value="${material.raw_Material_Id }" name="material" id="raw_Material_Id">
		<input type="hidden" value="${material_Stock.material_Id}" name="material_Id">
		
		 	<div class="layui-form-item" style="margin-top: 50px;">
				<label class="layui-form-label" style="width: 120px;">材料名称</label>
				 <div class="layui-input-inline">
					 <input type="text"  lay-verify="cpmc" autocomplete="off" class="layui-input bj" style="width: 800px;" value="${material.material_Name}" disabled="">
				</div>
			</div>
			...
			<div class="layui-form-item">
					...
					<div class="layui-inline" style="top:9px;left:-79px;">
					      ...
					      <div class="layui-input-inline">
					        <input type="text" name="stock" lay-verify="stock" autocomplete="off" class="layui-input bj" style="width: 200px;" value="${material_Stock.stock}" disabled="">
					      </div>
				    </div>
					  ...
					  <div class="layui-inline" style="top:9px;left: -130px;">
					      ...
					      <div class="layui-input-inline" style="width: 100px;">
					        <input type="text" autocomplete="off" class="layui-input bj" style="width: 100px;" id="scsl" disabled="" value="${material.numbers}">
					      </div>
				     </div>
			 ...
			 <div class="layui-form-item layui-form-text">
			    ...
			    <div class="layui-input-block">
			      <textarea placeholder="请输入内容" name="remarks"  lay-verify="remarks" id="remarks" class="layui-textarea" style="width:68.5%">${material_Stock.remarks}</textarea>
			    </div>
			 </div>
...
//监听提交
  form.on('submit(demo1)', function(data){
	...
	//加载所有未完全入库的材料
	function allWrkMaterial(form){
		$.ajax({
			type : "post",
			url : "<c:url value='/matStock/allWrkMaterial.do'/>",
			async : false,
			dataType : 'json',
			error : function() {
				alert("出错");
			...
			success : function(msg) {
				for(var i=0;i<msg.length;i++){
						$("#material").append(
					    	"<option value='"+msg[i].raw_Material_Id+"'>"+ msg[i].material_Name +"</option>"); 
				}
			}
		});
		 form.render('select');
	...
	function materialNumber(){
		...
		if(flag==false){
			...
			$.ajax({
				type : "post",
				url : "<c:url value='/matStock/queryMaterialById.do'/>",
				async : false,
				dataType : 'json',
