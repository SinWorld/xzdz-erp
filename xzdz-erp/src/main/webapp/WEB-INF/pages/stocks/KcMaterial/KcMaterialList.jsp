		 ...
		 <div class="layui-form-item">
		    <div class="layui-inline" style="width:325px;">
				...
				<div class="layui-input-inline" style="text-align: left">
					<select name="cl" id="cl" lay-filter="cl" lay-verify="cl" lay-search="">
						<option value="" selected="selected">请选择材料</option>
					</select>
				</div>
			</div>
...
layui.use(['table','form','layedit', 'laydate'], function(){
  ...
  table.render({
	    elem: '#test'
	    ,url:url+'kc_materialStock/kcList.do'
	    ,toolbar: '#toolbarDemo'
	    ,title: '材料库存列表'
	    ,totalRow: true
	    ,cols: [[
  ...
  $('#do_search').on('click', function () {
	...
	//ajax加载所有的材料
	function reloadCl(form){
		$.ajax({
			type : "post",
			url : "<c:url value='/stockRecod/allCLList.do'/>",
			async : false,
			dataType : 'json',
			error : function() {
				alert("出错");
	...
	//ajax加载所有的材料库位
	function reloadClKw(form){
		$.ajax({
			type : "post",
			url : "<c:url value='/stockRecod/allClKwList.do'/>",
			async : false,
			dataType : 'json',
			error : function() {
				alert("出错");
			...
			success : function(msg) {
				for (var i = 0; i < msg.length; i++) {
					$("#kw").append(
							"<option value='"+msg[i].material_Id+"'>"+ msg[i].stock +"</option>");
				}
				form.render('select');
			}
		});
	...
	function reloadWlId(form){
		$.ajax({
			type : "post",
			url : "<c:url value='/materielId/queryMatWlId.do'/>",
			async : false,
			dataType : 'json',
			error : function() {
				alert("出错");
