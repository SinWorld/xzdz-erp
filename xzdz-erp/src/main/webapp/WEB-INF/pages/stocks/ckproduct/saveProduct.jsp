...
<body style="width:100%;padding:0px; margin:0px;text-align: center;" onload="refreshAndClose()">
	<div style="width:1280px;height:auto;padding:0px; margin:0 auto;" id="main">
		<form class="layui-form" action='<c:url value="/ckStock/ckStock.do"/>' method="post">
		...
		<input type="hidden" id="flag" value="${flag}">
		 	...
		 	<div class="layui-form-item" style="margin-top: 50px;">
				...
				<div class="layui-input-inline" style="width: 800px;text-align: left;">
					<select name="stock_Id" id="stock_Id" lay-filter="stock_Id" lay-verify="stock_Id" lay-search="">
...
//ajax实现下拉项目带出项目的相关属性
	form.on('select(stock_Id)', function(data){
		//获得库存主键
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/ckStock/queryStockById.do'/>",
			async : false,
			dataType : 'json',
	...
	//加载所有未完全出库的库存
	function allWckProductStock(form){
		$.ajax({
			type : "post",
			url : "<c:url value='/ckStock/allWckProductStock.do'/>",
			async : false,
			dataType : 'json',
			error : function() {
				alert("出错");
			...
			success : function(msg) {
				for(var i=0;i<msg.length;i++){
						$("#stock_Id").append(
					    	"<option value='"+msg[i].stock_Id+"'>"+ msg[i].productName +"</option>"); 
				}
			}
		});
		 form.render('select');
