...
<body  style="width:100%;padding:0px; margin:0px;text-align: center;">
	<form class="layui-form">
		<div style="width:1280px;height:auto;padding:0px; margin:0 auto;" id="main">
			...
			<input type="hidden" value="${stock.stock_Id}" id="stock_Id">
			 <div class="layui-form-item" style="margin-top: 5%;margin-left:170px;">
			    ...
			    <div class="layui-input-block">
			      <input type="text" name="stock" lay-verify="stock" autocomplete="off" placeholder="产品名称" class="layui-input bj" style="width:50%" value="${stock.stock}" disabled="">
			    </div>
			 </div>
  ...
  var editIndex = layedit.build('LAY_demo_editor');
	...
	function allStock(transfer){
		...
		var stockId=$('#stock_Id').val();
			$.ajax({
				type : "post",
				url : "<c:url value='/ckStock/queryYrkProduct.do'/>",
				async : false,
				dataType : 'json',
	...
	function CreateTable(strs){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/ckStock/queryChoseProduct.do'/>",
			async : false,
			dataType : 'json',
			data:{"strs":strs,"stockId":stockId},
			error : function() {
				alert("出错");
			},
			success : function(msg) {
				...
				for(var i=0;i<msg.length;i++){
					...
					var addtr = $("<tr>"+
							...
							"<td><input type='text' class='form-control bj' aria-label='' aria-describedby='' value='"+msg[i].product_Name+"' disabled=''> <input type='hidden' name='product' value='"+msg[i].product_Id+"'></td>"+
							"<td><input type='text' class='form-control bj' aria-label='' aria-describedby=''  name='rksl' value='"+msg[i].rkNumber+"' disabled=''></td>"+
	...
	function removeRow(str){
		...
		for(var i=0;i<stockdms.length;i++){
			...
			for(var j=1;j<rows.length;j++){
				//主键
				var recId=$('input[name="product"]')[j-1].value;
				//若字符组中的主键等于表格中的隐藏主键则删除
				if(stockdms[i]==recId){
					var tr=$('input[name="product"]')[j-1].parentElement.parentElement;
	...
	function saveSubmit(){
		...
		for(var j=1;j<rows.length;j++){
			//获得成品主键
			var productId=$('input[name="product"]')[j-1].value;
			//获得填写的出库数量
