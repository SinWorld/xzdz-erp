...
<body  style="width:100%;padding:0px; margin:0px;text-align: center;">
	<form class="layui-form">
		<div style="width:1280px;height:auto;padding:0px; margin:0 auto;" id="main">
			...
			<input type="hidden" value="${material_Stock.material_Id}" id="material_Id">
			 <div class="layui-form-item" style="margin-top: 5%;margin-left:170px;">
			    ...
			    <div class="layui-input-block">
			      <input type="text" name="stock" lay-verify="stock" autocomplete="off" placeholder="产品名称" class="layui-input bj" style="width:50%" value="${material_Stock.stock}" disabled="">
			    </div>
			 </div>
  ...
  var editIndex = layedit.build('LAY_demo_editor');
	...
	function allStock(transfer){
		...
		var material_Id=$('#material_Id').val();
			$.ajax({
				type : "post",
				url : "<c:url value='/clckStock/queryYrkMaterial.do'/>",
				async : false,
				dataType : 'json',
	...
	function CreateTable(strs){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/clckStock/queryChoseProduct.do'/>",
			async : false,
			dataType : 'json',
			...
			success : function(msg) {
				...
				for(var i=0;i<msg.length;i++){
					...
					var addtr = $("<tr>"+
							...
							"<td><input type='text' class='form-control bj' aria-label='' aria-describedby='' value='"+msg[i].material_Name+"' disabled=''> <input type='hidden' name='materialId' value='"+msg[i].raw_Material_Id+"'></td>"+
