...
<body style="width:100%;padding:0px; margin:0px;"onload="refreshAndClose()">
	...
	<div style="width:1280px;height:auto;padding:0px; margin:0 auto;" id="main">
		<form class="layui-form" action='<c:url value="/checkDelivery/editCheckDelivery.do"/>' method="post"  enctype="multipart/form-data">
			...
			<input type="hidden" name="row_Id" value="${checkDelivery.row_Id}" id="row_Id">
		 ...
		 <div class="layui-form-item" style="margin-top: 3%;">
		    <div class="layui-inline" style="top:9px;">
			      ...
			      <div class="layui-input-inline">
			        <input type="text" name="createTime" id="createTime"  lay-verify="createTime" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" style="width: 280px;" value="${checkDelivery.startTime}">
			      </div>
		    </div>
		 </div>
		 ...
		 <div class="layui-form-item layui-form-text">
		    ...
		    <div class="layui-input-block">
		      <textarea placeholder="请输入内容" name="remarks"  lay-verify="remarks" id="remarks" class="layui-textarea" style="width:64.2%">${checkDelivery.remarks}</textarea>
		    </div>
		 </div>
...
//多文件列表示例
  ...
  ,uploadListIns = upload.render({
	...
	function  fjPageLoad(){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/checkDelivery/pageLoadFj.do'/>",
			async : false,
			dataType : 'json',
			data:{"row_Id":row_Id},
			error : function() {
	...
	function removeFile(obj){
		...
		layer.confirm('您确定要删除该附件么？', {
			  ...
			  title:'提示',icon:7},function(){
				  $.ajax({
						type : "post",
						url : "<c:url value='/checkDelivery/removeFj.do'/>",
						async : false,
						dataType : 'json',
