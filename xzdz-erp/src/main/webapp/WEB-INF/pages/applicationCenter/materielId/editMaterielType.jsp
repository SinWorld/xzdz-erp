...
<div style="height:auto;padding:0px; margin:0 auto;" id="main">
	<form class="layui-form" action="<c:url value='/materielType/editMaterielType.do'/>" method="post">
		<input type="hidden" id="url" value='<c:url value="/"/>'>
		<input type="hidden" id="flag" value="${flag}">
		<input type="hidden" name="id" value="${wj.id}"> 
		<input type="hidden" id="wjjId" value="${wjj.id}"> 
			<div class="layui-form-item" style="margin-top: 5%;">
				<label class="layui-form-label" style="width: 125px;">父节点名称</label>
			...
			<div class="layui-form-item" style="margin-top: 5%">
			    <label class="layui-form-label" style="width: 125px;">子节点名称</label>
 ...
 <script>
	...
	function initOrgTree(form) {
		$.ajax({
			type : "post",
			url : "<c:url value='/materielType/orgWJJTree.do'/>",
			async : false,
			dataType : 'json',
			error : function() {
				alert("出错");
