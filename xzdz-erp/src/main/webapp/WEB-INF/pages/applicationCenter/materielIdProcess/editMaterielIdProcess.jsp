...
<body style="width:100%;padding:0px; margin:0px;" onload="refreshAndClose()">
	<div style="width:1100px;height:auto;padding:0px; margin:0 auto;" id="main">
		<form class="layui-form" action='<c:url value="/materielIdProcess/editMaterielIdProcess.do"/>' method="post">
			...
			<div class="layui-form-item">
			     <div class="layui-inline" style="left: -15px;">
			  	...
			  	<div class="layui-inline" style="left: -30px;">
				      ...
				      <div class="layui-input-inline">
				        <input type="text" name="materielTypeName" lay-verify="materielTypeName" autocomplete="off" class="layui-input bj" id="ckdj" value="${materielId.materielTypeName}" disabled="">
				      </div>
			     </div>
			     ...
			     <div class="layui-inline">
				      ...
				      <div class="layui-input-inline">
				        <input type="text" name="materielNumberName" lay-verify="materielNumberName" autocomplete="off" class="layui-input bj" id="ckdj" value="${materielId.materielNumberName}" disabled="">
				      </div>
			     </div>
		   </div>
		   	...
		   	<div class="layui-form-item">
			     <div class="layui-inline" style="left: -15px;">
				      ...
				      <div class="layui-input-inline">
				        <input type="text" name="createTime" id="createTime"  lay-verify="createTime" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" style="width: 190px;">
				      </div>
			     </div>
...
layui.use(['form', 'layedit', 'laydate','upload'], function(){
  ...
  form.on('select(type)', function(data){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/materielIdProcess/wlIdbcf.do'/>",
			async : false,
			dataType : 'json',
...
//多文件列表示例
  ...
  fjPageLoad();
	...
	function  fjPageLoad(){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/materielIdProcess/pageLoadFj.do'/>",
			async : false,
			dataType : 'json',
			data:{"row_Id":row_Id},
			error : function() {
	...
	function removeFile(obj){
		...
		layer.confirm('您确定要删除该附件么？', {
			  ...
			  title:'提示',icon:7},function(){
				  $.ajax({
						type : "post",
						url : "<c:url value='/materielIdProcess/removeFj.do'/>",
						async : false,
						dataType : 'json',
