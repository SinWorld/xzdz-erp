		 ...
		 <div class="demoTable" style="background-color: #CAE1FF;display: none;" id="gjssq">
			...
			<div class="layui-form-item">
				 ...
				 <div class="layui-inline">
			      ...
			      <div class="layui-inline" style="left:7px;">
				      <label class="layui-form-label" style="width: 90px;">录入日期</label>
				      <div class="layui-input-inline" style="width: 100px;">
			        	<input type="text"  id="lrrq1" lay-verify="lrrq1" placeholder="yyyy-MM-dd" autocomplete="off" class="layui-input">
				      </div>
				      <div class="layui-form-mid">-</div>
				      <div class="layui-input-inline" style="width: 100px;">
			            <input type="text"  id="lrrq2" lay-verify="lrrq2" placeholder="yyyy-MM-dd" autocomplete="off" class="layui-input">
				      </div>
				   </div>
			
			   <button class="layui-btn" data-type="reload" type="button" id="do_search" style="margin-left:105px;">搜索</button>
...
layui.use(['table','form','layedit', 'laydate','tree','util'], function(){
  ...
  table.render({
    elem: '#test'
    ,url:url+'materielIdProcess/materielIdProcessList.do'
    ,toolbar: '#toolbarDemo'
    ,title: '物料Id评审列表'
    ,totalRow: true
    ,cols: [[
      ...
      ,{field:'ckdj', width:"11%", align:'right', title: '参考单价',totalRow: true}
      ,{field:'materielTypeName', width:"11%", align:'center', title: '物料Id类型'}
      ,{field:'materielNumberName', width:"12%", align:'center', title: '物料Id类型号'}
      ,{field:'createTime', width:"11%", align:'center', title: '录入日期',templet:'<div>{{ layui.util.toDateString(d.createTime, "yyyy-MM-dd") }}</div>'}
      ,{field:'approvalmc', width:"11%", align:'center', title: '审批状态'}
  ...
  table.on('toolbar(test)', function(obj){
    ...
    if(obj.event=='getCheckData'){
    	var fp_Url="/materielIdProcess/initFqsp.do";
	    //权限验证
		 $.ajax({
	    		type : "post",
	    		...
	    		success : function(data) {
	    			if(data.flag){
	    				 layer.open({
	    			      		...
	    			      		resize:false,
	    			      	    anim: 1,
	    			      	  	content:[url+"materielIdProcess/initFqsp.do",'yes']
    			    	 });
	    		}else{
  ...
  table.on('row(test)', function(obj){
    ...
    var fp_Url="/materielIdProcess/showMaterielIdProcess.do";
	//权限验证
	 $.ajax({
    		type : "post",
    		url : "<c:url value='/PermissionVerification/checkPermission.do'/>",
    		...
    		success : function(data) {
    			if(data.flag){
    				 layer.open({
    				  		...
    				  		resize:false,
    				  	    anim: 1,
    				  		content:[url+"materielIdProcess/showMaterielIdProcess.do?row_Id="+row_Id,'yes']
   					  });
    		}else{
				layer.alert("当前用户无此功能权限，请联系管理员授权!!!",{icon:7});
	    	}
  ...
  $('#do_search').on('click', function () {
      ...
      var ckj1=$('#ckj1').val();
      var ckj2=$('#ckj2').val();
      var wlIdms=$('#remarks').val();
      var beginTime=$('#lrrq1').val();
      var endTime=$('#lrrq2').val();
