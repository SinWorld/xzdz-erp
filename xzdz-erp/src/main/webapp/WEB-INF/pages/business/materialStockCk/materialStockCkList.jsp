...
layui.use(['table','form','layedit', 'laydate'], function(){
  ...
  table.render({
	    elem: '#test'
	    ,url:url+'materialStockCk/initMaterialStockCkList.do?xsddId='+xsddId
	    ,title: '材料库位'
	    ,cols: [[
		       {field:'index', width:"8%", title: '序号', sort: true,type:'numbers'}
	   	      ,{field:'material_Name', width:"17%",align:'center', title: '材料名称'}
	   	      ,{field:'is_allck', width:"5%", align:'center', title: '是否全部入库',hide:true}
	   	      ...
	   	      ,{field:'materielId', width:"15%", align:'center', title: '物料Id'}
	   	      ,{field:'materialQuality', width:"15%", align:'center', title: '物料质量'}
	...
	if(obj.event === 'ck'){
   		parent.layer.open({
        		...
        		shadeClose: false,
         		resize:false,
         	    anim: 1,
        	  	content:[url+"materialStockCk/initMaterialCk.do?raw_material_id="+raw_material_id,'yes']
      	  	});
    }
  });
	...
	function submitForm(){
		...
		layer.confirm('您已完成出库操作了么？', {
			  ...
			  title:'提示'},function(index){
				$.ajax({
					type : "post",
					url : "<c:url value='/materialStockCk/submitForm.do'/>",
					async : false,
					dataType : 'json',
					data:{"taskId":taskId},
					error : function() {
