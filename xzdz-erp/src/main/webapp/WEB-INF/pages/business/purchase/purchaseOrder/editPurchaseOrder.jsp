			 ...
			 <div class="layui-form-item">
			     ...
			     <div class="layui-inline" style="left: -100px;">
			      ...
			      <div class="layui-inline" style="left: -126px;">
				      ...
				      <div class="layui-input-inline">
				        <input type="text" name="pur_Date" id="pur_Date" lay-verify="pur_Date" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" style="width: 200px;" value="${dgrq}">
				      </div>
			    </div>
			 </div>
				  ...
				  <tbody>
			  		<c:forEach items="${purchaseList}" var="p">
				  		<tr>
							...
							<td><input type='text' class='form-control bj' aria-label='' aria-describedby=''  name='model' value="${p.model}" title="${p.model}" readonly='readonly' ></td>
							...
							<td><input type='date' class='form-control' name='delivery_date' value="${p.delivery_date}" title="${p.delivery_date}"></td>
							...
							<td><button type='button' class='layui-btn layui-btn-normal' onclick='MaterielIDInfo(this)'>物料ID详情</button> <button type='button' class='layui-btn layui-btn-danger' title='删除一行' onclick='deleteData(${p.pur_Id})'><i class='layui-icon'>&#xe640;</i></button></td>
						</tr>
				  	</c:forEach>
				  <tr>
				  		<td>
...
function addRow(){
	...
	function saveContractOrder(){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/purchaseOrder/saveOrEditPurchaseList.do'/>",
			async : false,
			contentType :"application/json;charsetset=UTF-8",//必须
			dataType : 'json',
	...
	function  fjPageLoad(){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/purchaseOrder/pageLoadFj.do'/>",
			async : false,
			dataType : 'json',
			data:{"row_Id":row_Id},
			error : function() {
	...
	function removeFile(obj){
		...
		layer.confirm('您确定要删除该附件么？', {
			  ...
			  title:'提示',icon:7},function(){
				  $.ajax({
						type : "post",
						url : "<c:url value='/purchaseOrder/removeFj.do'/>",
						async : false,
						dataType : 'json',
