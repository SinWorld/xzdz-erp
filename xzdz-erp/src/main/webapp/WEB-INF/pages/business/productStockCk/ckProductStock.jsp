			...
			<input type="hidden" value="${product.product_Id}" id="product_Id">
			 <div class="layui-form-item" style="margin-top: 5%;margin-left:170px;">
			    ...
			    <div class="layui-input-block">
			      <input type="text" name="product_Name" lay-verify="product_Name" autocomplete="off" placeholder="产品名称" class="layui-input bj" style="width:50%" value="${product.product_Name}" disabled="">
			    </div>
			 </div>
			 <div class="layui-form-item" style="margin-left:170px;">
			    <label class="layui-form-label" style="width: 120px;">生产数量</label>
			    <div class="layui-input-block">
			      <input type="text" name="" lay-verify="" autocomplete="off" placeholder="产品名称" class="layui-input bj" style="width:50%" value="${product.numbers}" id="numbers" disabled="">
			    </div>
			 </div>
						...
						<table class="table table-bordered" id="khlxrs" style="width:50%">
						  <thead>
						    <tr>
						      ...
						      <th scope="col" style="text-align: center;width: 30%">成品名称</th>
  ...
  var editIndex = layedit.build('LAY_demo_editor');
	...
	function allyrkMatStock(transfer){
		var product_Id=$('#product_Id').val();
			$.ajax({
				type : "post",
				url : "<c:url value='/productStockCk/queryAllStock.do'/>",
				async : false,
				dataType : 'json',
	...
	function CreateTable(strs){
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/productStockCk/queryChoseProduct.do'/>",
			async : false,
			dataType : 'json',
			data:{"strs":strs,"product_Id":product_Id},
			error : function() {
				alert("出错");
			},
			success : function(msg) {
				...
				for(var i=0;i<msg.length;i++){
					...
					var addtr = $("<tr>"+
							...
							"<td><input type='text' class='form-control bj' aria-label='' aria-describedby='' value='"+msg[i].stock+"' disabled=''> <input type='hidden' name='materialId' value='"+msg[i].cpId+"'></td>"+
							"<td><input type='text' class='form-control bj' aria-label='' aria-describedby=''  name='rksl' value='"+msg[i].kcl+"' disabled=''><input type='hidden' name='kwId' value='"+msg[i].stock_Id+"'></td>"+
	...
	function removeRow(str){
		...
		for(var i=0;i<stockdms.length;i++){
			...
			for(var j=1;j<rows.length;j++){
				//主键
				var recId=$('input[name="kwId"]')[j-1].value;
	...
	function ckNumberChange(obj){
		...
		if(cksl>rksl){
			obj.value=0;
			$('#submit').hide();
			return layer.alert("出库数量不得大于该成品的库存量!",{icon:7});
		}
		if(cksl<0){
			obj.value=0;
			$('#submit').hide();
	...
	function saveSubmit(){
		...
		var rows=tables[0].rows;
		if(rows.length==1){
			return layer.alert("当前不存在出库成品不允许提交!",{icon:7});
		}
		//创建入库数组
		...
		$.ajax({
			type : "post",
			url : "<c:url value='/ckStock/saveProStock.do'/>",
			async : false,
